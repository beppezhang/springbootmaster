			...
			$('#add').on('click', function() {
				layer.open({
					  ...
					  maxmin: true, // 允许屏幕最小化
//					  anim: 2,
					  content: ['app_version_add.html'] //iframe的url，no代表不显示滚动条
					});
			});
			/**新增     end*/
	...
	//                     url: 'datas/laytpl_laypage_data.json?v=' + new Date().getTime(), //地址
						url: hostIp + "/appVersion/getAppVersion.do?v=" + new Date().getTime(),// 请求服务器的url
						elem: '#content', //内容容器
						params: { //发送到服务端的参数
							appType: appType,
	...
	//									  time: 2000, //2秒后自动关闭
										  anim: 2,
										  content: ['app_version_add.html?appVersionId='+$(this).data('id')], //iframe的url，no代表不显示滚动条
										});
								});
							});
							...
							$('#content').children('tr').each(function() {
								...
								$that.children('td:last-child').children('a[data-opt=del]').on('click', function() {
									...
									// ajax 请求后台
										$.ajax({
											url : hostIp + "/appVersion/delete.do",
											method : 'post',
											dataType : 'json',
											data: {
												appVersionId: $(this).data('id') // 传到后台的参数，子窗口的form表单
											},
