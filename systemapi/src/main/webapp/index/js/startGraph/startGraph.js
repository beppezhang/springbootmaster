			layui.config({
 			...
 			var url =  hostIp + "/startGraph/getAllStartGraph.do?v=" + new Date().getTime();// 请求服务器的url
			// 初始化
			$(function(){
				getList(0);
			})
...
//			
			...
			function getList(onlineState){
				layui.use(['paging', 'layer', 'form', 'element'], function() {
	                ...
	                paging.init({
	                    openWait: true,
						url: hostIp + "/startGraph/getAllStartGraph.do?v=" + new Date().getTime(),
						elem: '#content', //内容容器
						params: { //发送到服务端的参数
							t: Math.random(),
//							bootDiagramId:bootDiagramId,
	...
	//									  time: 2000, //2秒后自动关闭
										  anim: 2,
										  content: ['startGraph_edit.html?id='+$(this).data('id')], //iframe的url，no代表不显示滚动条
										});
								});
	...
	//									  time: 2000, //2秒后自动关闭
										  anim: 2,
										  content: ['startGraph_edit.html?find=1&id='+$(this).data('id')], //iframe的url，no代表不显示滚动条
										});
								});
							...
							$('#content').children('tr').each(function() {
								...
								$that.children('td:last-child').children('a[data-opt=del]').on('click', function() {
									...
									// ajax 请求后台
										$.ajax({
											url : hostIp + "/startGraph/delete.do",
											method : 'post',
											dataType : 'json',
											data: {
												graphId: $(this).data('id'), // 传到后台的参数，子窗口的form表单
											},
											success : function(data) {
												if(data.code == "200"){
					...
					$('#add').on('click', function() {
						layer.open({
							  ...
							  maxmin: true, // 允许屏幕最小化
//							  anim: 2,
							  content: ['startGraph_edit.html'], //iframe的url，no代表不显示滚动条
							});
					});
					/**新增     end*/
