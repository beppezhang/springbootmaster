...
var exportList = function() {
	...
	var remark = $("#remark").val();
	var vo = "?sourceString=" + sourceString+"&createStartTime="+createStartTime+"&createEndTime="+createEndTime+"&remark="+remark;
	location.href = hostIp +"/statisticalChannel/exportNewGuestSummaryChannel.do"+vo;
};
...
$('#search').on('click', function() {
	...
	var createEndTime = $("#createEndTime").val();
	var remark = $("#remark").val();
	getList(sourceString, createStartTime, createEndTime, remark);
});
 ...
 * 加载列表
 */
function getList(sourceString, createStartTime, createEndTime, remark){
	layui.use(['paging', 'layer', 'form', 'element'], function() {
		...
		paging.init({
			openWait: true,
			url: hostIp + "/statisticalChannel/getNewGuestSummaryChannel.do?v=" + new Date().getTime(),// 请求服务器的url
			elem: '#content', //内容容器
			params: { //发送到服务端的参数
